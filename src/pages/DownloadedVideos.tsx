...
import { Trash2, Play, Download, HardDrive } from "lucide-react";
...
const DownloadedVideos = () => {
  ...
  const { videos, loading, storageUsed, deleteVideo, createBlobUrl, formatStorageSize } = useOfflineVideos();
  const [playingVideo, setPlayingVideo] = useState<string | null>(null);

  const handleDelete = async (id: string, title: string) => {
    const success = await deleteVideo(id);
    if (success) {
      toast({ title: "Đã xóa", description: `"${title}" đã được xóa khỏi bộ nhớ` });
    } else {
      toast({ title: "Lỗi", description: "Không thể xóa video", variant: "destructive" });
    }
  };
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="px-4 py-4 border-b border-border">
          <h1 className="font-semibold text-lg">Nội dung tải xuống</h1>
        </div>
              ...
              {videos.map((video) => (
                <div key={video.id} className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{formatStorageSize(video.size)}</p>
                  </div>
