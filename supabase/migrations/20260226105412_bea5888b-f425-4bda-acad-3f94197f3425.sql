...
BEGIN
  ...
  -- Total potential FUN (from reward_transactions) - NOW includes SIGNUP, FIRST_UPLOAD, WALLET_CONNECT, BOUNTY
  SELECT COALESCE(SUM(
    CASE 
      ...
      WHEN reward_type = 'SIGNUP' THEN 10
      WHEN reward_type = 'FIRST_UPLOAD' THEN 10
      WHEN reward_type = 'WALLET_CONNECT' THEN 5
      WHEN reward_type = 'BOUNTY' THEN COALESCE(amount, 0)
      ELSE 0
    END
  ), 0)
  ...
  -- Breakdown by action type (potential) - NOW includes new types
  ...
  FROM (
    SELECT 
      CASE 
        ...
        WHEN reward_type = 'SIGNUP' THEN 'SIGNUP'
        WHEN reward_type = 'FIRST_UPLOAD' THEN 'FIRST_UPLOAD'
        WHEN reward_type = 'WALLET_CONNECT' THEN 'WALLET_CONNECT'
        WHEN reward_type = 'BOUNTY' THEN 'BOUNTY'
        ELSE 'OTHER'
      END AS action,
  ...
  FROM (
    SELECT status, COUNT(*) AS count,
           COALESCE(SUM(CAST(REGEXP_REPLACE(calculated_amount_formatted, '[^0-9.]', '', 'g') AS numeric)), 0) AS total_fun
    FROM mint_requests
    GROUP BY status
    ORDER BY count DESC
  ) t;
