...
BEGIN
  ...
  -- Level mapping synced with documentation:
  -- Light Seed (0-99), Light Builder (100-249), Light Guardian (250-499), Light Leader (500-799), Cosmic Contributor (800+)
  v_level := CASE
    WHEN v_final_score >= 800 THEN 'cosmic'
    WHEN v_final_score >= 500 THEN 'leader'
    WHEN v_final_score >= 250 THEN 'guardian'
    WHEN v_final_score >= 100 THEN 'builder'
    ELSE 'seed'
  END;
  ...
  INSERT INTO light_score_ledger (
    ...
    integrity_penalty, final_light_score, level, rule_version, computed_at
  ) VALUES (
    p_user_id,
    'month',
    v_epoch_start,
