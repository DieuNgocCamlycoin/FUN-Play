...
-- Drop and recreate get_users_directory_stats with owner support
DROP FUNCTION IF EXISTS public.get_users_directory_stats();

CREATE OR REPLACE FUNCTION public.get_users_directory_stats()
RETURNS TABLE(
  user_id uuid, username text, display_name text, avatar_url text, wallet_address text, created_at timestamptz,
  banned boolean, avatar_verified boolean, pending_rewards numeric, approved_reward numeric,
  total_camly_rewards numeric,
  view_rewards numeric, like_rewards numeric, comment_rewards numeric, share_rewards numeric,
  upload_rewards numeric, signup_rewards numeric, bounty_rewards numeric, manual_rewards numeric,
  posts_count bigint, videos_count bigint, comments_count bigint, views_count numeric,
...
AS $$
  ...
  WHERE has_role(auth.uid(), 'admin') OR is_owner(auth.uid())
  ORDER BY rt.total_camly DESC NULLS LAST;
$$;

-- Toggle avatar verified
CREATE OR REPLACE FUNCTION public.toggle_user_avatar_verified(p_admin_id uuid, p_user_id uuid)
RETURNS boolean
...
DECLARE
  v_current boolean;
BEGIN
  IF NOT (has_role(p_admin_id, 'admin') OR is_owner(p_admin_id)) THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;
  SELECT COALESCE(avatar_verified, false) INTO v_current FROM profiles WHERE id = p_user_id;
  UPDATE profiles SET avatar_verified = NOT v_current, updated_at = now() WHERE id = p_user_id;
  RETURN NOT v_current;
END;
$$;

-- Freeze user rewards
CREATE OR REPLACE FUNCTION public.freeze_user_rewards(p_admin_id uuid, p_user_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
BEGIN
  IF NOT (has_role(p_admin_id, 'admin') OR is_owner(p_admin_id)) THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;
  UPDATE profiles SET pending_rewards = 0, updated_at = now() WHERE id = p_user_id;
END;
$$;

-- Wipe all user rewards
CREATE OR REPLACE FUNCTION public.wipe_user_rewards(p_admin_id uuid, p_user_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
BEGIN
  IF NOT (has_role(p_admin_id, 'admin') OR is_owner(p_admin_id)) THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;
  UPDATE profiles SET pending_rewards = 0, approved_reward = 0, total_camly_rewards = 0, updated_at = now() WHERE id = p_user_id;
END;
$$;
