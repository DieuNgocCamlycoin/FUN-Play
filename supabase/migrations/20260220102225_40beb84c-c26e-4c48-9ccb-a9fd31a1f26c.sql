...
-- 1. Update is_completed_profile to only check banned
CREATE OR REPLACE FUNCTION public.is_completed_profile(p profiles)
RETURNS boolean LANGUAGE sql IMMUTABLE AS $$
  SELECT COALESCE(p.banned, false) = false;
$$;

-- 2. Update get_public_users_directory: use is_completed_profile + priority ordering
CREATE OR REPLACE FUNCTION public.get_public_users_directory()
 RETURNS TABLE(id uuid, username text, display_name text, avatar_url text, avatar_verified boolean, created_at timestamp with time zone, total_camly numeric, claimed_camly numeric, available_camly numeric, view_rewards numeric, like_rewards numeric, comment_rewards numeric, share_rewards numeric, upload_rewards numeric, signup_rewards numeric, bounty_rewards numeric, manual_rewards numeric, posts_count bigint, videos_count bigint, comments_count bigint, total_views bigint, likes_count bigint, shares_count bigint, sent_count bigint, sent_total numeric, recv_count bigint, recv_total numeric, mint_count bigint, minted_total numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ORDER BY
    (p.avatar_url IS NOT NULL AND p.display_name IS NOT NULL AND LENGTH(TRIM(p.display_name)) >= 2) DESC,
    rt.total_camly DESC NULLS LAST;
END;
$function$;

-- 3. Recreate mv_top_ranking using is_completed_profile
DROP MATERIALIZED VIEW IF EXISTS mv_top_ranking;
CREATE MATERIALIZED VIEW mv_top_ranking AS
SELECT p.id, p.username, p.display_name, p.avatar_url, p.total_camly_rewards
FROM profiles p
WHERE is_completed_profile(p)
  AND COALESCE(p.total_camly_rewards, 0) > 0
ORDER BY p.total_camly_rewards DESC NULLS LAST
LIMIT 100;
