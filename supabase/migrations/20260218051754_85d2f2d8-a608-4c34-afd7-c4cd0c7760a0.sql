CREATE OR REPLACE FUNCTION public.trace_wallet_detective(p_wallet_address text, p_admin_id uuid)
 RETURNS TABLE(user_id uuid, username text, display_name text, avatar_url text, avatar_verified boolean, wallet_address text, total_amount numeric, tx_count bigint, created_at timestamp with time zone, banned boolean)
 LANGUAGE plpgsql
