...
-- Fix calculate_user_light_score: use 'month' for period column (check constraint requires day/week/month)
...
BEGIN
  ...
  -- Raw fallback with ALL action types
  SELECT
  ...
  -- Raw content score from pplp_ratings
  SELECT COALESCE(SUM(
    POWER(
  ...
  -- Use the greater of features vs raw
  ...
  -- Write to ledger (period = 'month' per check constraint)
