...
RETURNS TABLE(
  ...
  sent_count bigint, sent_total numeric, recv_count bigint, recv_total numeric,
...
BEGIN
  ...
  LEFT JOIN LATERAL (SELECT COUNT(*) AS mint_count, COALESCE(SUM(CAST(regexp_replace(calculated_amount_formatted, '[^0-9.]', '', 'g') AS numeric)),0) AS minted_total FROM mint_requests m WHERE m.user_id=p.id AND m.status='minted' AND calculated_amount_formatted IS NOT NULL AND calculated_amount_formatted != '') mr ON true
