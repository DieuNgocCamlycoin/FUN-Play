...
BEGIN
  ...
  SELECT
    ...
    -- Likes given: video likes + post likes + comment likes + post comment likes (reply interactions)
    ...
    (
      SELECT COUNT(DISTINCT action_type) FROM (
        ...
        UNION ALL
        SELECT 'reply_like' FROM comment_likes WHERE user_id = p.id AND created_at::date = p_date HAVING COUNT(*) > 0
        UNION ALL
        SELECT 'post_reply_like' FROM post_comment_likes WHERE user_id = p.id AND created_at::date = p_date HAVING COUNT(*) > 0
        UNION ALL
...
-- Also update calculate_user_light_score to count reply likes in fallback path
...
BEGIN
  ...
  -- Fallback: compute from raw tables (now includes reply likes)
